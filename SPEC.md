...
7. **Rung 2 — LLM layout profile.** On failure, send **1–2 sample pages**
   (never the whole table, §7.14) to the model and ask for a `LayoutProfile`
   under a strict schema. Re-parse, re-reconcile. At most
   `max_profile_attempts` (§7.15) attempts, each fed the previous residual.
...
8. **Rung 3 — vision transcription** (scanned pages only): transcribe
   **one page at a time**, each page verified locally (§7.14).
...
3. **Period anchors**, matched case-insensitively: a line carrying a
   ...
   the page (§7.16); a `statement period` / `statement date` line whose
...
4. **`LayoutProfile`** is the only thing that varies between statements. It
   ...
   derives one without a model is §7.18.
...
12. **A row whose amount is zero is not a transaction** and is skipped, with
...
13. **Description** is the text between the date and the first amount
    ...
    description.
14. **The model never transcribes what cannot be verified.** Rung 2 sees at
...
15. **Bounds:** `max_profile_attempts = 3`; `max_llm_calls_per_statement`
...
16. **Account identity** is read from the first page of the period:
...
17. **Provider is an installation parameter.** `LLM_PROVIDER` selects
    `anthropic` (default) / `bedrock` / `vertex` / `foundry`; all four expose
    the same `messages.create`. No code path depends on which is chosen.

18. **Heuristic profile derivation (rung 0)**, from word coordinates alone:
    1. **Candidate rows** are lines whose leading words parse as a date under
       any candidate format (§7.16, plus the yearless formats of §7.11) and
