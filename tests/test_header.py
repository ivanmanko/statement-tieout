...
SPEC §7.5 (labels, scope, counts) and §7.19 (bank, account, period). ADR-001
...
class TestLetterheadBySize:
    """SPEC §7.19 — the bank name is the biggest text, not the first tidy line."""
...
class TestAccountMaskGuard:
    """SPEC §7.19 — `Box` ends in `x`, and `P.O.Box 4887` is not an account number."""
...
class TestLetterheadIsWordsNotLines:
    """SPEC §7.19 — a logo is set larger than the address printed beside it."""
...
class TestAccountMaskNeedsTwoCharactersOrNoGap:
    """SPEC §7.19 — OCR of `P.O.Box 4887` yields `P.O.B 0 x 4887` on some pages."""
...
class TestStatementCyclePair:
    """SPEC §7.19 — core-banking statements state the cycle as two lines."""
...
class TestAccountNumberWithoutAMask:
    """SPEC §7.19 — `ACCOUNT NUMBER 0011016426` means the account ends 6426."""
...
class TestPeriodFromBalanceLines:
    """SPEC §7.19 — the header may state only a statement date; the balances state the range."""
