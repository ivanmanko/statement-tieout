"""Heuristic layout profiling — rung 0 of the ladder (SPEC §7.21).
...
class TestBalanceColumnIsMeasuredNotAssumed:
    def test_a_rightmost_column_that_is_not_a_running_balance_stays_an_amount(self):
        """SPEC §7.21.4 — the chain has to actually hold."""
        noisy = rows_page(
...
class TestColumnsAreFoundByAlignmentNotFrequency:
    """SPEC §7.21.3 — the rule that a real statement disproved.
...
class TestTableHeadersAreNotSectionHeadings:
    """SPEC §7.21.6 — the column header carries the section vocabulary verbatim."""
...
class TestProfileComesFromTheDensestTablePage:
    """SPEC §7.21 — a binder holds pages whose layout is not the table's.
