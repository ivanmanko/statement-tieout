...
LETTERHEAD_LINES = 15
"""How far down the page a letterhead may be (SPEC §7.18)."""
...
def _read_bank(lines: Sequence[_Line]) -> str | None:
    """The letterhead: the words set in the largest type (SPEC §7.18).
...
def read_last4(lines: Sequence[RawLine]) -> str | None:
    """The masked or labelled account tail (SPEC §7.18). Also a period anchor."""
...
def read_period(lines: Sequence[RawLine]) -> DateRange:
    """The stated statement period (SPEC §7.18). Also a period anchor."""
    resolved = _as_lines(lines)
    for line in resolved:
...
def _cycle_pair(lines: Sequence[_Line]) -> DateRange:
    """`... : LAST STATEMENT` over `... : THIS STATEMENT` (SPEC §7.18)."""
    start = end = None
    for line in lines:
        squashed = _squash(line.text)
        dates = find_dates(line.text)
