"""Deriving a layout profile from coordinates alone — rung 0 (SPEC §7.21).
...
def derive_profile(pages: list[Page]) -> LayoutProfile | None:
    ...
    Derived from the single densest table page (SPEC §7.21) and applied to all
    ...
    if not amounts:
        return None
    amounts = amounts[-2:]  # at most two, rightmost first (SPEC §7.21.5)
...
class _Cluster:
    ...
    def is_a_column(self) -> bool:
        """SPEC §7.21.3: amounts in a table are aligned; amounts in a sentence are not."""
...
def _behaves_like_a_running_balance(
    ...
    """SPEC §7.21.4: b[i] − b[i−1] == ± the row's amount, on a majority of pairs."""
...
def _classify_headings(
    ...
    """SPEC §7.21.6: a line sitting over the money columns is the table's own header."""
